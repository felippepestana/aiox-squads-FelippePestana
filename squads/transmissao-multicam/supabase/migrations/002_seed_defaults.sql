...
-- Singleton tables: obs_settings_singleton and meet_settings_singleton are
-- expression-based UNIQUE INDEXes (CREATE UNIQUE INDEX ... ON ... ((true))),
-- not named CONSTRAINTs. PostgreSQL's `ON CONFLICT ON CONSTRAINT <name>`
-- requires a real constraint, so we use index expression inference instead:
-- `ON CONFLICT ((true)) DO NOTHING` matches the unique index directly.
