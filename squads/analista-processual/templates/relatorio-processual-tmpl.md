<!--
  ╔════════════════════════════════════════════════════════════════════════════╗
  ║ FONTE CANÔNICA da estrutura do relatório (seções obrigatórias por modo).     ║
  ║ Este template é a ÚNICA fonte de verdade para o que cada modo                ║
  ║ (MODO_PROCESSUAL / MODO_JURIDICO) deve conter. Os arquivos abaixo apenas      ║
  ║ resumem/validam esta estrutura e devem ser mantidos em sincronia:            ║
  ║   - agents/documentador-processual.md        (estrutura embutida por modo)   ║
  ║   - tasks/gerar-relatorio-processual.md       (action items por modo)        ║
  ║   - checklists/relatorio-completeness-checklist.md (itens obrigatórios)      ║
  ║   - ARCHITECTURE.md                           (resumo do tier de síntese)    ║
  ╚════════════════════════════════════════════════════════════════════════════╝
-->

# Relatório {{modo}} — {{titulo_demanda}}

> **Gerado em:** {{data}} | **Squad:** Analista Processual v1.0 | **Modo:** {{modo}}

---

<!-- =================== MODO_PROCESSUAL =================== -->
<!-- Use esta seção quando MODO_PROCESSUAL. Remova se MODO_JURIDICO. -->

## Identificação

| Campo | Valor |
|-------|-------|
| **Processo/Demanda** | {{titulo}} |
| **Data da Análise** | {{data}} |
| **Responsável pela Análise** | Squad Analista Processual |

## Sumário Executivo

{{resumo_executivo}}

## Mapeamento do Processo

| # | Etapa | Ator | Entradas | Saídas | Critério de Conclusão |
|---|-------|------|----------|--------|---------------------|
| 1 | {{etapa_1}} | {{ator_1}} | {{entradas_1}} | {{saidas_1}} | {{criterio_1}} |

## Pontos de Decisão

| # | Decisão | Condição Verdadeira | Condição Falsa |
|---|---------|---------------------|----------------|

## Avaliação de Maturidade

**Score:** {{score_maturidade}}/5 — **Nível:** {{nivel_maturidade}}

**Justificativa:** {{justificativa_maturidade}}

## Top-5 Riscos

| # | Risco | Probabilidade | Impacto | Mitigação |
|---|-------|---------------|---------|----------|
| 1 | {{risco_1}} | {{prob_1}} | {{impacto_1}} | {{mitigacao_1}} |

## Gargalos Identificados

| # | Etapa | Tipo de Gargalo | Impacto Estimado |
|---|-------|----------------|------------------|

## Roadmap de Melhorias

### Imediato (até 30 dias)

{{roadmap_imediato}}

### Médio Prazo (90 dias)

{{roadmap_90_dias}}

### Longo Prazo

{{roadmap_longo_prazo}}

---
*Analista Processual Squad v1.0 | Gerado por documentador-processual*

<!-- =================== MODO_JURIDICO =================== -->
<!-- Use esta seção quando MODO_JURIDICO. Remova se MODO_PROCESSUAL. -->

## Identificação do Processo

| Campo | Valor |
|-------|-------|
| **Número** | {{numero_processo}} |
| **Classe** | {{classe_processual}} |
| **Vara/Tribunal** | {{vara_tribunal}} |
| **Partes** | Autor: {{autor}} \| Réu: {{reu}} |
| **Fase Atual** | {{fase_atual}} |
| **Último Ato** | {{ultimo_ato}} — {{data_ultimo_ato}} |
| **Data da Análise** | {{data}} |

## Resumo Executivo

{{resumo_executivo}}

## Histórico Processual

| Data | Ato | Autor/Órgão | Observação |
|------|-----|------------|------------|

## Questões Jurídicas

**Principal:** {{questao_principal}}

**Subsidiárias:**
- {{questao_subsidiaria_1}}

## Fundamentação Legal

### Legislação Aplicável

| Dispositivo | Relevante Para |
|------------|----------------|

### Jurisprudência Relevante

| Tribunal | Número | Data | Tese/Ementa |
|---------|--------|------|------------|

### Súmulas Aplicáveis

| Tribunal | Súmula | Enunciado |
|---------|--------|----------|

## Análise de Mérito

### Pontos Fortes

{{pontos_fortes}}

### Vulnerabilidades

{{vulnerabilidades}}

### Riscos Jurídicos

{{riscos_juridicos}}

## Análise Estratégica

| Cenário | Descrição | Probabilidade | Condições |
|---------|-----------|---------------|----------|
| Otimista | {{desc_otimista}} | {{prob_otimista}}% | {{cond_otimista}} |
| Realista | {{desc_realista}} | {{prob_realista}}% | {{cond_realista}} |
| Pessimista | {{desc_pessimista}} | {{prob_pessimista}}% | {{cond_pessimista}} |

**Viabilidade de Acordo:** {{viabilidade_acordo}} — {{justificativa_acordo}}

## Orientações Práticas

### Ações Urgentes (até 7 dias)

- [ ] {{acao_urgente_1}} — **Prazo:** {{prazo_urgente_1}}

### Plano de Ação (4-8 semanas)

| Semana | Ação | Objetivo | Responsável |
|--------|------|----------|------------|

### Monitoramento

| Item | O que observar | Frequência |
|------|---------------|------------|

### Para o Cliente

{{orientacoes_cliente}}

## Conclusões e Recomendações

{{conclusoes}}

---

```citacoes
documento: {{doc_1}}
trecho: {{trecho_1}}
tipo: {{tipo_1}}
---
```

---
*Analista Processual Squad v1.0 | Gerado por documentador-processual*
