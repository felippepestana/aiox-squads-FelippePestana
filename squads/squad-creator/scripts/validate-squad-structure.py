...
def _security_match_is_documentation_noise(line: str) -> bool:
    """Skip grep/pattern/example lines so doc anti-patterns do not fail T1-SEC."""
    low = line.lower()
    if re.search(r"\bgrep\s+-r", low):
        return True
    if "pattern:" in low or "patterns:" in low:
        return True
    if "action:" in low and "grep" in low:
        return True
    if "scan_command:" in low:
        return True
    stripped = line.strip()
    if stripped.startswith("-") and any(
        k in low for k in ("api_key:", "apikey", "secret:", "password =", "postgres://", "mongodb://", "redis://", "mysql://")
    ):
...
def grep_in_file(file_path: Path, pattern: str) -> List[str]:
    """Search for pattern in file; return full matches (not only capture groups)."""
    if not file_path.exists():
        return []
    try:
        with open(file_path, "r", encoding="utf-8") as f:
        ...
        for m in re.finditer(pattern, content, re.MULTILINE | re.IGNORECASE):
            ...
            if _security_match_is_documentation_noise(line):
                continue
            full = m.group(0)
            low_full = full.lower()
            if any(
