...
def _resolve_bash() -> str | None:
    """Prefer Git Bash on Windows; avoid WSL/System32 bash stubs that break repo paths."""
    env_bash = os.environ.get("GIT_BASH_EXE") or os.environ.get("AIOX_BASH")
    ...
    if not found:
        return None
    low = found.lower()
    if platform.system() == "Windows" and ("system32" in low or "windowsapps" in low):
        return None
    return found
...
def _run_validator(squad_name: str) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        [BASH_EXE, str(VALIDATE_SCRIPT), squad_name, "--quick", "--json"],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
        check=False,
...
@pytest.mark.skipif(BASH_EXE is None, reason="Git Bash (or AIOX_BASH/GIT_BASH_EXE) required; WSL bash.exe is not supported for these tests")
