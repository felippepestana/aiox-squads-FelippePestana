...
/**
 * Forensic device specialist — UC-LP-007.
 * Ported from web/server/pericia.ts so the iPhone judicial assessment laudo is
 * available inside the unified Next.js product. `generateLaudo` is pure string
 * templating; `assistQuesito` runs through the shared LLM gateway.
 */
    ...
    messages: [
      {
        role: "system",
        content:
          "Você é um perito judicial especializado em dispositivos iPhone, com conhecimento em eletrônica, CDC e normas ABNT. Responda quesitos de forma técnica, direta e fundamentada, citando evidências concretas. Máximo 3 parágrafos.",
      },
      { role: "user", content: `DADOS DIAGNÓSTICOS:\n${summary}\n\nQUESITO: ${quesito}` },
    ],
    temperature: 0.3,
    max_tokens: 800,
  });
...
export class ForensicDeviceSpecialist {
  name = "Perito Forense de Dispositivo";
  description = "Diagnóstico técnico-pericial de iPhone e geração de laudo (UC-LP-007)";

  generateLaudo = generateLaudo;
  assistQuesito = assistQuesito;
}

export const forensicDeviceSpecialist = new ForensicDeviceSpecialist();
