...
const serverDir = path.dirname(fileURLToPath(import.meta.url));
{
  ...
  if (!fs.existsSync(envPath)) envPath = path.join(serverDir, "../../.env");
...
function routeSessionId(req: express.Request): string {
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      const client = new Anthropic({ apiKey });
      const uploaded: UploadedFile = await uploadFileFromBuffer(
        client,
        file.buffer,
        file.originalname,
        mimeType
